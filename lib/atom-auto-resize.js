...
export default {
  ...
  calculateWidth(currentSize) {
    ...
    this.getTextEditorRectanglesSortedByLeftBorder().forEach(rect => {
      ...
      if (rect.right > currentSize.width) {
        // Text editor is outside of Atom window right border
      ...
      if (prev && prev.right > rect.left) {
        // Text editor overlaps with the previous text editor
        w = w + Math.ceil(prev.right - rect.left);
      }
      if (rightDockRectangle && rect.right > rightDockRectangle.left) {
